...
for page_no in range(1, request_total_num +1):
    ...
    df.rename(columns={'adtFrDd': '운영가능일자', 'sgguNm': '시군구명', 'sidoNm': '시도명', 'spclAdmTyCd': '구분코드', 'telno': '전화번호',
                       'yadmNm': '기관명'}, inplace=True)
    df = df[['기관명', '구분코드', '시군구명', '시도명', '전화번호', '운영가능일자']]
